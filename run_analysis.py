...
def run():
    ...
    # TODO
    results = {"Symbol": [], "EPS Next 5Y in percent": [], "DCF Value": [
    ], "Free Cash Flow": [], "Beta": [], "Totoal debt": []}
    for index, row in df.iterrows():
        ...
        model.set_FCC_growth_rate(
            short_term_growth_rate, medium_term_growth_rate, long_term_growth_rate)

        fair_value = model.calc_fair_value()
        results["Symbol"].append(row['Symbol'])
        results["EPS Next 5Y in percent"].append(row['EPS Next 5Y in percent'])
        results["DCF Value"].append(fair_value)
        results["Free Cash Flow"].append(stock.get_free_cashflow())
        results["Totoal debt"].append(stock.get_total_debt())
        results["Beta"].append(stock.get_beta())
        print("Done analysis : ", row['Symbol'])
    ...
    print("Done saving the output into the csv file...")
    # ....
